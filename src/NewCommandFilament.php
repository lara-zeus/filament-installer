...
class NewCommandFilament extends Command
{
    ...
    protected function configure()
    {
        $this
            ...
            ->addOption('migrate', null, InputOption::VALUE_NONE, 'run migration')
            ->addOption('database', null, InputOption::VALUE_NONE, 'database type')
    ...
    protected function interact(InputInterface $input, OutputInterface $output)
    {
        ...
        if (! $input->getOption('migrate')) {
            $input->setOption('migrate', confirm(label: 'Would you like to run migration after the installation completed?', default: true));
        }
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if (($process = $this->runCommands($commands, $input, $output))->isSuccessful()) {
            if ($name !== '.') {
                ...
                spin(
                    fn () => $this->configureDefaultDatabaseConnection($directory, $database, $name, $migrate),
            ...
            if ($input->getOption('git') || $input->getOption('github') !== false) {
                spin(
                    fn () => $this->createRepository($directory, $input, $output),
                    'preparing git ...'
                );
            }
            ...
            if ($input->getOption('pest')) {
                spin(
                    fn () => $this->installPest($directory, $input, $output),
                    'installing pest ...'
                );
            }
            ...
            $output->writeln('<fg=gray>➜</> Password:<options=bold>123123123</>');
            $output->writeln('');
    ...
    protected function installFilament(string $directory, InputInterface $input, OutputInterface $output)
    {
        ...
        $this->runCommands([$this->phpBinary().' artisan make:filament-user --name=Admin --email="admin@'.$name.'.com" --password=123123123'], $input, $output, workingPath: $directory);
